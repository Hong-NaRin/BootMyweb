...
//        // 파일
//        try {
//            result = FileCopyUtils.copyToByteArray(file); // 파일데이터의 byte 배열값을 구해서 반환
//
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
...
//    }
    ...
    // ResponEntity를 이용해서 이미지 응답 문서를 작성
    @GetMapping("/display/{filepath}/{uuid}/{filename}")
    public ResponseEntity<byte[]> display(@PathVariable("filepath") String filepath,
        ...
        try {
            byte[] arr = FileCopyUtils.copyToByteArray(file); // 파일 데이터의 byte배열값을 구해서 반환
    ...
    public ResponseEntity<byte[]> download (@PathVariable("filepath") String filepath,
        ...
        try {
            ...
            result = new ResponseEntity<>(fileCopy, header, HttpStatus.OK); // 바디에 담을 내용, 헤더에 담을 내용, 상태메시지
